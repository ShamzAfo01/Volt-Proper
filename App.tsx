...
// A helper to render the pixel grid decoration based on the "Frames" logic
const PixelGridIcon = ({ variant = 'default' }: { variant?: 'default' | 'orange' }) => {
  ...
  // Creating a 6x6 grid visual based on the "Frames" provided
  return (
    <div className="grid grid-cols-6 gap-[2px] w-[34px] h-[34px] p-[1px]">
       {/* Row 1 */}
       <div className={highlight}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div>
       {/* Row 2 */}
       <div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div>
       {/* Row 3 */}
       <div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div>
       {/* Row 4 */}
       <div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div><div className={color}></div>
  ...
  return (
    <div className="min-h-screen w-full bg-black overflow-x-hidden relative">
      ...
      <section className="relative w-full h-[900px] flex items-center justify-center overflow-hidden">
        {/* Unicorn Studio effect fills the hero */}
      ...
      {/* BELOW THE FOLD – keep only the decorative bars, text removed */}
      ...
      <div className="absolute top-[1031px] right-[120px] hidden xl:flex gap-4 p-2 bg-[#050505]">
        <div className="grid grid-cols-6 gap-1 w-[34px] h-[34px]">
          {Array.from({ length: 36 }).map((_, i) => (
            <div
              key={i}
              className={`w-1 h-1 ${i === 0 ? 'bg-white' : 'bg-[#323232]'}`}
            ></div>
          ))}
        </div>
      </div>
