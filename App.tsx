  ...
  return (
    <div className="grid grid-cols-6 gap-[2px] w-[34px] h-[34px] p-[1px]">
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={`r2-${i}`} className={color}></div>
      ))}
      {/* Row 3 */}
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={`r3-${i}`} className={color}></div>
      ))}
      {/* Row 4 */}
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={`r4-${i}`} className={color}></div>
      ))}
      {/* Row 5 */}
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={`r5-${i}`} className={color}></div>
      ))}
      {/* Row 6 */}
      <div className={highlight}></div>
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={`r6-${i}`} className={color}></div>
      ))}
    </div>
  );
};
      ...
      <section className="w-full bg-[#050505] py-24 md:py-32">
        <div className="max-w-[1440px] mx-auto px-6 md:px-8">
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[433px,minmax(0,1fr)]">
            {/* Left: bar chart card */}
            <div className="bg-[#121212] rounded-[3px] p-8 relative overflow-hidden border border-[#1b1b1b]/40">
              <p className="font-mono text-[18px] md:text-[20px] leading-[1.2] tracking-[-0.03em] text-white">
                Instant Reconciliation.
              </p>

              <div className="mt-10 flex items-end gap-3 h-[230px]">
                {bars.map((h, i) => (
                  <div
                    key={i}
                    className="w-4 rounded-[1px] bg-gradient-to-t from-[#FE4E1D] to-[#F26A45]"
                    style={{ height: `${h}px` }}
                  />
                ))}
              </div>
